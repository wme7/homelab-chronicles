...
In other words, we would develop an educated `prompt` that can be used by commercial AI to assist on compiling a guide (or develop fully automated scripts) to build the mini-cluster I wish to have.

Steps:
- An initial prompt was manually written [here](./documents/prompt.md). 
- This prompt was used to generate a [guide](./documents/guide.md) (in markdown format) and a set of fully automated scripts to build the cluster.
- The guide was reviewed manually and the scripts were tested.
- The prompt is updated with new assumptions and requirements as problems are encountered.
- The process is repeated until the cluster is fully operational.
...
> __Note__: The cluster is fully operational as it passes all the initial stress tests, but clearely some fine tunning of the SLURM configuration is needed as the AI is making to many assumptions on the Pi's hardware and software stack.

> __Note__: The guide and scripts are available [here](./documents/guide.md) and [here](./documents/scripts.md).